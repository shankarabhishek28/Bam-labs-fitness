...
import { deleteOneHabit, editHabitName, getAllHabits } from "@/serviceAPI/tennant";
...
const HabitsTable = ({ data, loading, fetchAllHabits, setNewHabit }) => {
    ...
    return (
        <div className="pt-2 ">
            {editHabit && <div style={{ zIndex: 9999 }} className="fixed top-0 left-0 w-screen bg-[rgba(0,0,0,0.5)] h-screen flex items-center justify-center backdrop-blur-sm z-20">
                <div className="w-[408px] md:w-[450px] bg-white px-5 py-7 gap-3 rounded-xl flex flex-wrap shadow">
                    ...
                    <label className="flex flex-wrap w-full">
                        ...
                        <Input name="habitName" value={habitName} onChange={(e) => setHabitName(e.target.value)} className='w-full border border-[#D1D1D1]' />
                    </label>


                    <div className='w-full mt-2 flex items-center justify-between'>
                        <Button variant='outline' onClick={() => setEditHabit(false)}>Discard</Button>
                        <Button
                            onClick={() => handleEditHabit(currentHabit?._id)}
                            disabled={habitName === currentHabit?.name}
                            className={`${habitName === currentHabit?.name
                        ...
                        >
                            ...
                            <TableCell>
                                ...
                                <div className="flex items-center justify-start gap-8">
                                    <Button onClick={() => editHabitModal(item)} className=' px-2 h-8 text-[14px] flex gap-2 w-[90px]'>
                                        <Pencil color="white" size={20} />Edit</Button><Button onClick={() => deleteHabit(item?._id)} className='px-2 h-8 text-[14px] flex gap-2 items-center w-[90px]'><Trash2 size={20} color="white" /> Delete</Button>
                                </div>
